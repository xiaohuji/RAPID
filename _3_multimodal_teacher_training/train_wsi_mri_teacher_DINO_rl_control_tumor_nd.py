...
from _3_multimodal_teacher_training.datasets.dataset import RCCDatasetWSITumor
from _3_multimodal_teacher_training.config.acmil_mri_config import get_parser
...
def train(dataloader, model, criterion, optimizer, device, epoch, args):
    model.train()
    ...
    for step, batch in enumerate(bar):
        ...
        fuse_auc, fuse_acc, _, _, _, _, _, _, presnet_t = get_cm(np.array(trues), np.array(preds_fuse))
...
if __name__ == '__main__':
    ...
    args.mask_drop = 0.7
    args.n_masked_patch = 200
    args.n_token = 5
    args.mini_batch = 1
    args.Tmax = 20
    args.weight_decay = 1e-4

    lr_wsi_backbones = [1e-5]
    lr_wsi_heads = [1e-5]
    lr_news = [1e-4]
    ...
    for lr_wsi_backbone in lr_wsi_backbones:
        args.lr_wsi_backbone = lr_wsi_backbone
        for lr_wsi_head in lr_wsi_heads:
            args.lr_wsi_head = lr_wsi_head
            for lr_new in lr_news:
                args.lr_new = lr_new
                ...
                train_data_info = train_data_info.merge(
                    train_data_info_wsi[['pid', 'img_name']].rename(
                        columns={'img_name': 'wsi_img_name'}),
                    on='pid',
                    how='left'
                )
                train_data_info.dropna(subset='wsi_img_name', inplace=True)

                train_val_pair_wsi_info_train, train_val_pair_wsi_info_val = train_test_split(
                    train_data_info,
                    test_size=0.2,
                    stratify=train_data_info[label_col_name],  # 按目标列分层
                    random_state=seed
                )
                ...
                total_negative_samples = np.count_nonzero(
                    train_val_pair_wsi_info_train[label_col_name].values == 0)
                total_positive_samples = np.count_nonzero(
                    train_val_pair_wsi_info_train[label_col_name].values == 1)
                logit_pos_weight = torch.tensor(
                    [total_negative_samples / total_positive_samples]).to(device)
                ...
                print(f"[Init LR groups] {get_current_lrs(optimizer)}")
                ...
                for epoch in range(args.epochs):
                    ...
                    train_metrics = train(train_dataloader, model, criterion,
                                                     optimizer, device, epoch, args)
                    AUC_list.append(train_metrics['fuse_auc'])
                    epoch_record['train_AUC'] = train_metrics['fuse_auc']
                    ...
                    if epoch % 1 == 0:
                        ...
                        filename = (f"../checkpoints/ck_wsi_mri/ep{epoch}_nmp{args.n_masked_patch}_nt{args.n_token}_md{args.mask_drop}"
                                    f"_wd{args.weight_decay}_mb{args.accum_steps}_ln{args.lr_new}_lw{args.lr_wsi_backbone}_lwb{args.lr_wsi_backbone}")
