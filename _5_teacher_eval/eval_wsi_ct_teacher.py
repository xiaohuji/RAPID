...
from _5_teacher_eval.config.acmil_ct_config import get_parser
...
def load_pretrained_wsi_branch(model, ckpt_path: str, device: str):
    """
    将单模态 WSI 的 ACMIL_GA 权重加载到 teacher 中：
        dimreduction.*  -> dimreduction.*
        attention.*     -> attention.*
        classifier.*    -> classifier.*
        Slide_classifier.* -> wsi_head.*
    """
...
if __name__ == '__main__':
    ...
    pretrained_dict = torch.load(r"", map_location='cuda:' + str(args.device))['model']
